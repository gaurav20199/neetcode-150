class Solution {
    ...
    public Pair longestPalindromicSubstring(String s, int startingIdx, int endingIdx,Pair [][]dp) {
        ...
        if(isCharEqual) {
            ...
            Pair innerPair = longestPalindromicSubstring(s, startingIdx + 1, endingIdx - 1, dp);
            
            // If inner window is empty (like "bb") or is itself a valid palindrome
            if (innerLength == 0 || innerPair.maxLength == innerLength)
                return dp[startingIdx][endingIdx] = new Pair(endingIdx - startingIdx + 1, s.substring(startingIdx, endingIdx + 1));
        }
        ...
        return dp[startingIdx][endingIdx]=pairWithoutStartingChar.maxLength>=pairWithoutEndingChar.maxLength?pairWithoutStartingChar:pairWithoutEndingChar;     
